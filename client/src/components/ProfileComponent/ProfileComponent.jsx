...
const ProfileComponent = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`https://blog-website-4zc7.vercel.app//profile/${email}`, {
        headers: {
  ...
  const onLogOut = async () => {
    try {
      const response = await axios.get(`https://blog-website-4zc7.vercel.app//auth/logout`);
  ...
  const ImageUploader = async () => {
    try {
      const response = await axios.patch(`https://blog-website-4zc7.vercel.app//profile/${email}`, imgData, {
        headers: {
          'x-access-token': localStorage.getItem('token'),
