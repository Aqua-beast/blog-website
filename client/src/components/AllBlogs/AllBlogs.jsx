...
function AllBlogs() {
    ...
    const deleteHandle = async (title) => {
        try {
            const res = await axios.delete(`https://blog-website-server-tau.vercel.app/articles/delete/${title}`, {
    ...
    useEffect(() => {
        axios.get('https://blog-website-server-tau.vercel.app/articles', {
            headers: {
                "x-access-token": localStorage.getItem("token"),
            },
        })
